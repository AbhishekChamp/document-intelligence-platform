...
export const DashboardPage: React.FC = () => {
  ...
  const { issueStats, scoreBreakdown, trends, topIssues, improvement } =
    useAnalysisStats(currentAnalysis, allAnalyses);

  const issueTypeData = Object.entries(issueStats.byType).map(
    ([type, count]) => ({
  ...
  const severityData = Object.entries(issueStats.bySeverity).map(
    ([severity, count]) => ({
  ...
  const scoreData = scoreBreakdown
    ? [
        ...
        {
          name: "Correctness",
          score: scoreBreakdown.correctness,
          fullMark: 100,
        },
  ...
  const avgScore =
    allAnalyses.length > 0
      ? Math.round(
          allAnalyses.reduce((sum, a) => sum + a.score.overallScore, 0) /
            allAnalyses.length,
        )
      : 0;
              ...
              <CardContent>
                <div className="flex flex-col items-center py-6">
                  ...
                  <div className="mt-6 text-center">
                    <Badge
                      variant={
                        currentAnalysis.score.overallScore >= 70
                          ? "success"
                          : "warning"
                      }
                      className="text-sm px-4 py-1 rounded-full"
                    >
                      {currentAnalysis.score.overallScore >= 80
                        ? "Excellent"
                        : currentAnalysis.score.overallScore >= 60
                          ? "Good"
                          : "Needs Improvement"}
                    </Badge>
                  </div>
                  {improvement && (
                    <div className="flex items-center justify-center gap-4 mt-6 px-4 py-2 bg-white dark:bg-gray-800 rounded-xl">
                      <div
                        className={`flex items-center gap-1 ${improvement.scoreDelta >= 0 ? "text-emerald-600" : "text-rose-600"}`}
                      >
                        ...
                        <span className="font-semibold">
                          {improvement.scoreDelta > 0 ? "+" : ""}
                          {improvement.scoreDelta.toFixed(1)}
                        </span>
                      </div>
        ...
        <Card className="py-20">
          <CardContent className="text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6 max-w-md mx-auto">
              Analyze a document to see detailed insights and trends on your
              dashboard
            </p>
            <a
              href="/"
