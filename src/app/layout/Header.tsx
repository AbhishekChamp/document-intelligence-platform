...
const HeaderComponent: React.FC<HeaderProps> = ({ sidebarCollapsed }) => {
  const { currentAnalysis, isAnalyzing, analysisProgress, analysisProgressMessage } = useStore();
        ...
        {isAnalyzing && (
          <div className="flex items-center gap-3 px-4 py-2 bg-violet-50 dark:bg-violet-900/20 rounded-xl shrink-0">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-xs text-violet-500">{Math.round(analysisProgress)}%</span>
            </div>
          </div>
        )}
        {!isAnalyzing && lastAnalyzedText && (
          <div className="flex items-center gap-2 text-sm truncate">
            <span className="text-gray-500 dark:text-gray-400 shrink-0">Last analyzed:</span>
