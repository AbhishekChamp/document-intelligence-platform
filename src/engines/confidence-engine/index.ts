import type { ValidationEngine, ValidationIssue, NormalizedDocument, DocumentScore, ReadabilityMetrics } from '../../shared/types/domain.types';
...
export class ConfidenceEngine implements ValidationEngine {
  ...
  async analyze(_document: NormalizedDocument): Promise<ValidationIssue[]> {
    return [];
  }
  ...
  async analyzeWithEngines(
    ...
    for (const engine of this.engines) {
      ...
      try {
        ...
        const avgIssueConfidence = engineIssues.length > 0
          ? engineIssues.reduce((sum, i) => sum + i.confidence, 0) / engineIssues.length
          : 1;
  ...
  private calculateScores(
    ...
    if (metrics) {
      ...
      const sentencePenalty = Math.min(30, Math.max(0, metrics.averageSentenceLength - 15) * 2);
      const paragraphDensity = metrics.wordCount / Math.max(1, metrics.paragraphCount);
      const paragraphPenalty = Math.min(20, Math.max(0, paragraphDensity - 50) * 0.2);
      const complexPenalty = metrics.complexWordPercentage * 0.3;
      const passivePenalty = metrics.passiveVoicePercentage * 0.5;
      
      clarityScore = Math.max(0, Math.min(100, 
        50 + (fleschContribution * 0.4) - sentencePenalty - paragraphPenalty - complexPenalty - passivePenalty
      ));
    }

    const spellIssues = results.find(r => r.engine.includes('Spell'))?.issues.length || 0;
    const grammarIssues = results.find(r => r.engine.includes('Grammar'))?.issues.length || 0;
    ...
    const complianceIssues = results.find(r => r.engine.includes('Compliance'))?.issues || [];
    const highSeverityCompliance = complianceIssues.filter(i => i.severity === 'high').length;
    const mediumSeverityCompliance = complianceIssues.filter(i => i.severity === 'medium').length;
    
    const compliancePenalty = (highSeverityCompliance * 15) + (mediumSeverityCompliance * 5);
    const complianceScore = Math.max(0, Math.min(100, 100 - compliancePenalty));

    let confidenceScore = results.length > 0
